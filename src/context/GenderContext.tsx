...
// The student-facing lessons are anonymous (not tied to a logged-in siswa), so
// gender is a local UI preference used to pick which illustration set to show
// (D4 in docs/spec/lessons.md, follows D1's local-per-device model).
export type Gender = "male" | "female";

const STORAGE_KEY = "gema-gender";

// localStorage-backed store read via useSyncExternalStore — hydration-safe
// (server + first client render both use "male") without a setState-in-effect,
// and re-renders when the preference changes.
