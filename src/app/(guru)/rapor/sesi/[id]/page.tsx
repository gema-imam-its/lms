...
export default async function DetailSesiRapor({
  ...
  const imamsData = session.imams as unknown;
  const student = (
    Array.isArray(imamsData) ? imamsData[0] : imamsData
  ) as { id: string; nama: string; kelas?: string } | null;
  ...
  if (isCancelled) {
    ...
    subtitleMessage =
      "Latihan ini dihentikan sebelum selesai — sebagian gerakan mungkin belum sempat terekam.";
  ...
  return (
    <div className="-mx-6 -my-6">
      {/* Top Banner (Score) */}
      <div
        className={`w-full ${headerColor} pt-12 pb-24 px-4 relative overflow-hidden rounded-t-3xl`}
      >
        <div className="max-w-4xl mx-auto relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 text-white">
          <div className="flex-1">
            ...
            <div className="inline-block bg-white/20 px-6 py-3 rounded-2xl font-gohan text-xl backdrop-blur-sm">
              Skor Keseluruhan:{" "}
              <span className="font-bold text-3xl ml-2">{score}%</span>
            </div>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-12 relative z-20">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-md mb-8 flex flex-wrap gap-8 justify-between">
          <div>
            ...
            <div className="font-gohan text-2xl text-gema-navy">
              {student?.nama || "-"}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div
              className={`font-gohan text-2xl ${isCancelled ? "text-gray-500" : "text-green-600"}`}
            >
          ...
          <div>
            ...
            <div className="font-gohan text-2xl text-orange-500">
              {session.total_kesalahan_imam} kali
            </div>
          </div>
        </div>
        ...
        <h2 className="font-gohan text-2xl text-gray-700 mb-6">
          Detail Evaluasi Tiap Gerakan
        </h2>

        {movementsError ? (
          <div className="bg-white p-10 rounded-3xl text-center shadow-sm border-2 border-red-100">
            <p className="font-gilroy text-red-500">
              Gagal memuat rincian gerakan: {movementsError.message}
            </p>
          </div>
        ) : !movements || movements.length === 0 ? (
          <div className="bg-white p-10 rounded-3xl text-center shadow-sm">
            <p className="font-gilroy text-gray-500">
              Belum ada rincian gerakan terekam.
            </p>
          </div>
        ) : (
          <div className="bg-white rounded-3xl shadow-sm p-6 md:p-10 mb-10">
            <div className="relative border-l-4 border-gray-100 ml-6 md:ml-8 space-y-10 py-4">
              {movements.map((mov) => {
                ...
                if (isCorrect) {
                  feedbackText =
                    "Sempurna! Anda menahan postur ini dengan tenang (Tuma'ninah tercapai).";
                } else if (isWrong) {
                  feedbackText =
                    "Gerakan terlalu cepat (Tuma'ninah belum tercapai).";
                } else {
                  // isNeutral (Transisi seperti Sedekap, Berdiri)
                  feedbackText =
                    "Postur transisi terekam dengan durasi yang wajar.";
                }

                // Kalau ada gerakan_menyimpang, tambahkan sebagai catatan perbaikan
                if (
                  mov.gerakan_menyimpang &&
                  mov.gerakan_menyimpang.length > 0
                ) {
                ...
                if (
                  gerakanClean.includes("berdiri") ||
                  gerakanClean.includes("takbir")
                )
                  imgUrl = "/images/modul/gerakan-berdiri.png";
                if (
                  gerakanClean.includes("sedekap") ||
                  gerakanClean.includes("fatihah")
                )
                  imgUrl = "/images/modul/gerakan-sedekap.png";
                if (gerakanClean.includes("rukuk"))
                  imgUrl = "/images/modul/gerakan-rukuk.png";
                if (gerakanClean.includes("itidal"))
                  imgUrl = "/images/modul/gerakan-itidal.png";
                if (gerakanClean.includes("sujud"))
                  imgUrl = "/images/modul/gerakan-sujud.png";
                if (
                  gerakanClean.includes("duduk") ||
                  gerakanClean.includes("tahiyat")
                )
                  imgUrl = "/images/modul/gerakan-duduk.png";
                if (gerakanClean.includes("salam"))
                  imgUrl = "/images/modul/gerakan-salam.png";
                ...
                return (
                  <div key={mov.id} className="relative pl-8 md:pl-12">
                    ...
                    <div
                      className={`border-2 rounded-2xl p-6 flex flex-col md:flex-row gap-6 transition-all ${
                        isWrong
                    ...
                    >
                      ...
                      <div className="w-24 h-24 relative bg-white rounded-xl border-2 border-gray-100 p-2 shrink-0 self-start md:self-center">
                        <Image
                          src={mov.foto_pose_url || imgUrl}
                          alt={mov.nama_gerakan}
                          fill
                          className="object-contain"
                        />
                      </div>
                      ...
                      <div className="flex-1">
                        <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center mb-2 gap-2">
                          <h3 className="font-gohan text-2xl text-gema-navy capitalize flex items-center gap-3">
                            ...
                            <span className="text-sm font-gilroy text-gray-400 font-normal mt-1">
                              ({mov.entry_time} -{" "}
                              {mov.exit_time ||
                                (mov.exit_reason === "CANCELLED"
                                  ? "Dibatalkan"
                                  : mov.exit_reason === "NORMAL_FINISH"
                                    ? "Selesai"
                                    : "-")}
                              )
                            </span>
                        ...
                        <p
                          className={`font-gilroy text-lg ${isWrong ? "text-orange-600" : "text-gray-600"}`}
                        >
                          {feedbackText}
                        </p>
                        ...
                        <div className="mt-4 flex gap-4 text-xs font-gilroy text-gray-400 border-t border-gray-100 pt-3">
                          {mov.hip_angle != null && (
                            <span>Panggul: {mov.hip_angle}°</span>
                          )}
                          {mov.knee_angle != null && (
                            <span>Lutut: {mov.knee_angle}°</span>
                          )}
                          {mov.arm_angle != null && (
                            <span>Lengan: {mov.arm_angle}°</span>
                          )}
                        </div>
                      </div>
                    </div>
