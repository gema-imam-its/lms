...
// Drop-in replacement for <Image> for the prayer-movement illustrations. Shows
// the gender-specific art and, if that variant is missing, falls back once to
// the flat original — so a lesson never renders a broken image while art is
// filled in.
export default function ModulImage({ src, alt, ...rest }: ModulImageProps) {
  const { gender } = useGender();
  const genderedSrc = toGenderedSrc(src, gender);

  // Track only which gendered path failed; the shown src is derived from
  // that, so no effect is needed. When gender changes, genderedSrc changes
  // and the new variant is attempted automatically.
